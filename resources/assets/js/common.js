                            ...
                            if (response.status == 'OK' && status=='success') {
                                ...
                                if(response.posts.per_page){
                                    ...
                                    for(var i=0; i < response.posts.data.length; i++){
                                        $postList.append(tplPost({post: response.posts.data[i], auth_check: response.auth_check}));

                                        $postList.find(_o.postShortItem).filter(':last').PostBtnDelete().find('[data-toggle="tooltip"]').tooltip();
                                    }

                                    if(response.posts.prev_page_url!==null){
                                        tplBtnPrevLink = _.template( $(_o.btnSimplePrevTplLink).html() );
                                        $naviRoot.find(_o.btnSimplePrevWrap).empty().prepend(tplBtnPrevLink({prev_page_url: response.posts.prev_page_url}))
                                    } else {
                                        tplBtnPrevEmpty = _.template( $(_o.btnSimplePrevTplEmpty).html() );
                                        $naviRoot.find(_o.btnSimplePrevWrap).empty().prepend(tplBtnPrevEmpty);
                                    }

                                    if(response.posts.next_page_url!==null){
                                        $btn.data('page', response.posts.current_page + 1);
                                        tplBtnNextLink = _.template( $(_o.btnSimpleNextTplLink).html() );
                                        $naviRoot.find(_o.btnSimpleNextWrap).empty().prepend(tplBtnNextLink({next_page_url: response.posts.next_page_url}))
                                    } else {
                                    ...
                                    tplPagesData = {
                                        ...
                                        "per_page": response.posts.per_page,
                                        "current_page": response.posts.current_page,
                                        "last_page": response.posts.last_page,
                                        "next_page_url": response.posts.next_page_url,
                                        "prev_page_url": response.posts.prev_page_url,
                                        "from": response.posts.from,
                                        "to": response.posts.to
                                    };


                                    $naviRoot.find(_o.navPagesEl).empty().prepend(tplNavPages(tplPagesData));

                                    history.pushState('', '', uriPage + response.posts.current_page);
                                }
